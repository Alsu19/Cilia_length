...
merge_by_keywords <- function(result_lists) {
  # Define the keywords for categories
  keywords <- c("Dorsal", "Ventral", "Nasal", "Temporal")
  ...
  for (list_name in names(result_lists)) {
    ...
    merged_results[[list_name]] <- list(Dorsal = list(), Ventral = list(), Nasal = list(), Temporal = list())
    ...
    for (i in seq_along(category_data)) {
      ...
      # Loop through each keyword (Dorsal, Ventral, Nasal, Temporal)
  ...
  for (list_name in names(merged_results)) {
    ...
    for (keyword in keywords) {
      if (keyword %in% names(category_data)) {
        ...
        # Add a new column to indicate the keyword (Dorsal, Ventral, Nasal, Temporal)
        df$Category <- keyword
...
plot_segment_length <- function(merged_results) {
  ...
  for (list_name in names(merged_results)) {
    ...
    if (nrow(combined_data) > 0) {
      ...
      # Check Kruskal-Wallis test result
      kruskal_result <- kruskal.test(`Segment Length` ~ Category, data = combined_data)
      print(kruskal_result)
      
      # Run Dunn's test if Kruskal-Wallis is significant
      if (kruskal_result$p.value < 0.05) {
        dunn_result <- dunnTest(`Segment Length` ~ Category, data = combined_data, method = "bonferroni")
        print(dunn_result)
        
        # Extract Dunn's p-values and significant comparisons
        dunn_pvalues <- dunn_result$res$`P.adj`
        
        # Identify significant comparisons (p-value < 0.05)
        significant_comparisons <- which(dunn_pvalues < 0.05)
        
        # Check if there are significant comparisons
        if (length(significant_comparisons) > 0) {
          # Format Dunn's significant comparisons as a string for display
          dunn_text <- paste(
            paste(dunn_result$res$Comparison[significant_comparisons], 
                  "p =", format(dunn_pvalues[significant_comparisons], digits = 3), collapse = "\n"), 
            collapse = "\n"
          )
        } else {
          dunn_text <- "No significant differences between categories."
        }
      } else {
        dunn_text <- "No significant differences found with Kruskal-Wallis test."
      }
      
      # Create a p-value string to display in the plot title
      p_value_text <- paste("Kruskal-Wallis p-value:", format(kruskal_result$p.value, digits = 3))
      ...
      p_boxplot <- ggplot(combined_data, aes(x = Category, y = `Segment Length`, color = Category)) +
        ...
        labs(title = paste("Boxplot of Segment Length -", list_name),
             subtitle = paste(p_value_text, "\n", dunn_text),  # Add Kruskal-Wallis and Dunn results
             x = "Category (Dorsal, Ventral, Nasal, Temporal)", 
             y = "Segment Length, µm") +
        theme_minimal() + 
...
plot_segment_length(merged_lists)
...
# Function to plot the number of files containing keywords (Dorsal, Ventral, Nasal, Temporal) for each category
